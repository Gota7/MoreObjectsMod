...
namespace
{
	...
	constexpr unsigned SPRITE_ID = 0x16; //Should be 2e.
...
int Magikoopa::Shot::Behavior()
{
	...
	if(wmClsn.IsOnGround() || wmClsn.IsOnWall() || /*ADD CHECK FOR CEILING*/ (actor && actor->actorID != KAMEK && actor->actorID != KAMELLA))
	{
		res->TrackRes();
		Actor* newActor;
...
void Magikoopa::OnTurnIntoEgg(Player& player)
{
	if(player.Unk_020bea94())
		Unk_0201061c(player, NUM__COINS_MAGIKOOPA, 0);
	else
