...
namespace
{
	using StateFuncPtr = void(Goomba2::*)();
	...
	const short SMALL_ID = 0x0215;
	const short NORMAL_ID = 0x0216;
	const short LARGE_ID = 0x0217;
}

SharedFilePtr Goomba2::modelFile;
SharedFilePtr Goomba2::texSeqFile;
SharedFilePtr Goomba2::animFiles[Goomba2::NUM_ANIMS];

SpawnInfo<Goomba2> Goomba2::spawnDataNormal =
{
	&Goomba2::Spawn,
	0x00c8,
	0x0018,
...
SpawnInfo<Goomba2> Goomba2::spawnDataSmall =
{
	&Goomba2::Spawn,
	0x00c9,
	0x0019,
...
SpawnInfo<Goomba2> Goomba2::spawnDataBig =
{
	&Goomba2::Spawn,
	0x00ca,
	0x001a,
...
//0212bff8
Goomba2* Goomba2::Spawn()
{
	return new Goomba2;
}

int Goomba2::InitResources()
{
	...
	extraDamage = charBehav == 2 ? 8 : 0;
	extraSpeed = charBehav == 0 ? 5 : 1;
...
//021296cc
void Goomba2::SpawnSilverStarIfNecessary()
{
	if(spawnSilverStar == 1)
	{
		UntrackStar();
...
void Goomba2::KillAndSpawnSilverStar()
{
...
//02129168
void Goomba2::RenderRegurgGoomba2Helpless(Player* player)
{
	regurgTimer = 0x3c;
...
//021294d0
void Goomba2::KillIfTouchedBadSurface()
{
...
bool Goomba2::UpdateIfEaten()
{
	...
	if(eatState != 0)
	{
		if(eatState == 1)
		{
			...
			{
				RenderRegurgGoomba2Helpless(eater);
				horzSpeed = -0xf000_f;
				speed.y = 0x14000_f;
		...
		if(isBeingSpit)
		{
			...
			if(wmClsn.JustHitGround())
				RenderRegurgGoomba2Helpless(nullptr);
		}
...
void Goomba2::GetHurtOrHurtPlayer()
{
		...
		if(!(hitFlags & 0x8000) && player->actorID == 0x00bf && !killedByOtherMeans)
		{
			...
			else if(state == 0 || state == 4 || state == 5)
			{
				if(sizeType == SizeType::SMALL)
				{
					...
					{
						player->Hurt(pos, 0 + extraDamage, 0xc000_f + 0x6000_f, 1, 0, 1);
					}
				...
				else if(cylClsn.hitFlags & 0x400000)
				{
					...
					{
						player->Hurt(pos, sizeType + extraDamage, 0xc000_f + 0x6000_f, 1, 0, 1);
					}
...
//02129238
void Goomba2::KillIfIntoxicated()
{
	if(wmClsn.IsOnGround())
		return;
...
void Goomba2::UpdateTargetDirAndDist(Fix12i theMaxDist)
{
...
void Goomba2::State0HelperFunc()
{
	...
	if(!redirected) //0212af14 else
	{
		if(distToPlayer < maxDist)
		{
			...
			if(charBehav < 2 && charBehav != 0 && distToPlayer <= JUMP_DIST && rigMdl.anim.file == animFiles[RUN].filePtr)
			{
				state = 4;
...
unsigned Goomba2::OnYoshiTryEat()
{
	switch(sizeType)
	{
		case SizeType::SMALL:
...
void Goomba2::OnTurnIntoEgg(Player& player)
{
...
Fix12i Goomba2::OnAimedAtWithEgg()
{
	switch(sizeType)
	{
		case SizeType::BIG:
